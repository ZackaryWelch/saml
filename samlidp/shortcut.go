...
func (s *Server) HandleGetShortcut(w http.ResponseWriter, r *http.Request) {
	shortcut := Shortcut{}
	id := s.idFromPath(r.URL.Path)
	err := s.Store.Get(fmt.Sprintf("/shortcuts/%s", id), &shortcut)
	if err != nil {
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}
...
func (s *Server) HandlePutShortcut(w http.ResponseWriter, r *http.Request) {
	...
	shortcut.Name = s.idFromPath(r.URL.Path)

	err := s.Store.Put(fmt.Sprintf("/shortcuts/%s", shortcut.Name), &shortcut)
	if err != nil {
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}
...
func (s *Server) HandleDeleteShortcut(w http.ResponseWriter, r *http.Request) {
	err := s.Store.Delete(fmt.Sprintf("/shortcuts/%s", s.idFromPath(r.URL.Path)))
	if err != nil {
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}
...
func (s *Server) HandleIDPInitiated(w http.ResponseWriter, r *http.Request) {
	shortcutName := s.propFromPath(r.URL.Path, "shortcut")
	shortcut := Shortcut{}
	...
	case shortcut.URISuffixAsRelayState:
		relayState = s.propFromPath(r.URL.Path, "*")
	}
